function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords)
		...
		if minimal then
			...
			for k,v in ipairs(self.accounts) do
				...
				if accounts[v.name] then
					v.money = exports['qs-inventory']:GetItemTotalAmount(self.source, v.name)
				end
			end
		...
		if money >= 0 then
			...
			if account then
				...
				if accounts[accountName] then
					exports['qs-inventory']:SetInventoryItems(self.source, accountName, newNoney)
				end
			end
		end
	end
	...
	self.addAccountMoney = function(accountName, money)
		if money > 0 then
			...
			if account then
				...
				local accounts = exports['qs-inventory']:GetAccounts()
				if accounts[accountName] then
					exports['qs-inventory']:AddItem(self.source, accountName, newNoney)
				end
			end
		end
	end
	...
	self.removeAccountMoney = function(accountName, money)
		if money > 0 then
			...
			if account then
				...
				local accounts = exports['qs-inventory']:GetAccounts()
				if accounts[accountName] then
					exports['qs-inventory']:RemoveItem(self.source, accountName, newNoney)
				end
			end
		end
	end
	...
	self.addInventoryItem = function(name, count, metadata, slot)
		exports['qs-inventory']:AddItem(self.source, name, count or 1, slot, metadata)
	end

	self.removeInventoryItem = function(name, count, metadata, slot)
		exports['qs-inventory']:RemoveItem(self.source, name, count or 1, slot, metadata)
	end

	self.setInventoryItem = function(name, count, metadata)
		exports['qs-inventory']:SetInventoryItem(self.source, name, count, metadata)
	end
	...
	self.canCarryItem = function(name, count, metadata)
		exports['qs-inventory']:CanCarryItem(self.source, name, count)
	end
